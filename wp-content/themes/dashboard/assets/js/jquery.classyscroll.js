...
(function($) {
  function _ClassyScroll(c, s) {
    ...
    this.container
      ...
      .contents()
      .filter(this.settings.contentFilter)
      .wrapAll('<div class="scrollbar-content"></div>');
    ...
    if (this.settings.scroll == "horizontal") {
      ...
      this.container.prepend(
        this.vpath.append(this.vslider),
        ...
        this.fixFn(function() {
          if (!this.pressed) {
            this.sliders
              .stop()
              .fadeTo(
                this.settings.sliderOpacityTime,
  ...
  _ClassyScroll.prototype.init = function() {
    ...
    this.bindEvent(this.container, "touchmove", function(e) {
      ...
      this.scroll(
        this.touch.sv + (this.touch.sy - b.pageY) * this.settings.touchSpeed,
        this.touch.sh + (this.touch.sx - b.pageX) * this.settings.touchSpeed
      );
      a.preventDefault();
      a.stopPropagation();
    });
    this.bindEvent(this.container, "touchend touchcancel", function(e) {
      ...
      this.sliders
        .stop()
        .fadeTo(this.settings.sliderOpacityTime, this.settings.sliderOpacity);
      a.stopPropagation();
    });
    ...
    this.bindEvent($(window), "resize", function() {
      ...
      this.scroll(
        Math.round(
          (parseInt(this.vslider.css("top"), 10) * this.vpath.height()) / g
        ),
        Math.round(
          (parseInt(this.hslider.css("left"), 10) * this.hpath.width()) / htrack
        )
      );
      g = this.vpath.height();
      htrack = this.hpath.width();
    ...
    this.bindEvent(this.container, "mousewheel", function(e, a, b, c) {
      this.scroll(
        this.vslider.position().top - this.settings.wheelSpeed * c,
        this.hslider.position().left + this.settings.wheelSpeed * b,
        e
      );
      this.sliders.stop().fadeTo(this.settings.sliderOpacityTime, 1);
    ...
    this.bindEvent(this.container, "uscrollbar", function(e, v, h, a) {
      if (v === "reset") {
        this.container
          .find(".scrollbar-content, .scrollbar-handle")
          .stop()
          .css({
            top: 0,
            left: 0
